# Arroz en Uruguay

## Descripción general

El **arroz** (*Oryza sativa*) es uno de los cultivos más emblemáticos del Uruguay agropecuario. Aunque el país no figura entre los grandes productores mundiales por volumen, se destaca por su **altísima calidad**, su eficiencia tecnológica y por ser uno de los exportadores de arroz con mayor porcentaje de su producción destinada al mercado externo. Uruguay exporta entre el **85% y el 95%** de su producción de arroz, siendo un exportador neto de grano de alta calidad.

---

## Historia del arroz en Uruguay

| Período | Hito |
|---------|------|
| Fines siglo XIX | Primeros cultivos experimentales de arroz en zonas inundables del este |
| 1920–1940 | Cultivos comerciales incipientes en los departamentos del este (Treinta y Tres, Cerro Largo) |
| 1940–1960 | Expansión significativa; primeros sistemas de riego por bombeo; molinos arroceros |
| 1960–1975 | Consolidación de la cuenca arrocera del este; adopción de variedades mejoradas |
| 1975–1985 | Tecnificación acelerada; introducción del control de nivel en taipas; cooperativas arroceras |
| 1985–2000 | Gran salto de productividad; variedades de INIA; siembra directa; cosechadoras especializadas |
| 2000–2015 | Uruguay entre los exportadores más eficientes del mundo; precio premium por calidad |
| 2015–2023 | Producción estabilizada; inversión en valor agregado; diversificación de destinos |

---

## Zonas productoras

La producción arrocera uruguaya se concentra casi exclusivamente en la **zona este del país**, asociada a la cuenca de la **Laguna Merín** y sus tributarios:

| Departamento | Participación aproximada en la superficie |
|-------------|------------------------------------------|
| **Treinta y Tres** | ~35–40% de la superficie arrocera |
| **Cerro Largo** | ~25–30% |
| **Rocha** | ~15–20% |
| **Artigas** (norte) | ~5–10% (cuenca río Uruguay) |
| **Paysandú / Salto** | ~3–5% |

La región este dispone de:
- Suelos **vertisólicos** profundos y de alta fertilidad, aptos para el arroz irrigado.
- Abundante disponibilidad de agua superficial en las cuencas de los ríos Olimar, Tacuarí, Yaguarón y en la Laguna Merín.
- Topografía plana que facilita el manejo del agua y la mecanización.

---

## Sistema de producción

El arroz uruguayo se cultiva íntegramente bajo **riego por inundación controlada**:

1. **Preparación del suelo:** nivelación laser-guiada para conseguir desniveles mínimos; fangueo en algunos sistemas.
2. **Siembra:** mayoritariamente en siembra directa sobre rastrojo; también en seco con incorporación posterior de agua.
3. **Fertilización:** nitrógeno fraccionado (pre-siembra + macollaje + panoja); fósforo y potasio según análisis.
4. **Manejo del agua:** inundación desde emergencia hasta maduración; drenaje en cosecha. Las **taipas** (diques de tierra) controlan el nivel del agua en cada potrero.
5. **Control de malezas:** herbicidas pre y post emergentes; el **arroz rojo** (*Oryza sativa* tipo silvestre) es la principal maleza de difícil control.
6. **Cosecha:** con cosechadoras adaptadas para terrenos húmedos; generalmente septiembre a diciembre según el año.

---

## Variedades

Uruguay invierte fuertemente en el desarrollo de variedades propias adaptadas al ecosistema del este:

| Variedad | Origen | Características |
|----------|--------|----------------|
| **INIA Tacuarí** | INIA Uruguay | Long grain; alta calidad molinera y culinaria; referencia del mercado |
| **INIA Olimar** | INIA Uruguay | Long grain; mayor rendimiento; muy difundida |
| **INIA Merín** | INIA Uruguay | Alta productividad; buena calidad |
| **INIA Caraguatá** | INIA Uruguay | Mayor resistencia a enfermedades |
| **El Paso 144** | Ancestral | Histórica; gran calidad culinaria; referencia para mercados exigentes |
| **Bluebelle** | EE.UU. | Histórica; ya en desuso pero fue la base del desarrollo arrocero |

El INIA (Instituto Nacional de Investigación Agropecuaria), a través de su **Estación Experimental de Treinta y Tres ("José Enrique Ávila")**, es el principal generador de variedades arroceras uruguayas.

---

## Productividad

Uruguay tiene una de las **productividades más altas del mundo** en arroz irrigado:

| Indicador | Valor aproximado |
|-----------|-----------------|
| Rendimiento promedio | 7.500–8.500 kg/ha (arroz cáscara) |
| Superficie cultivada | 150.000–200.000 ha/año (varía según precio internacional) |
| Producción total | 1.200.000–1.600.000 toneladas/año (arroz cáscara) |
| Exportaciones | 700.000–1.000.000 toneladas/año (arroz elaborado) |
| Valor exportaciones | USD 300–500 millones/año |

*Fuente: DIEA-MGAP / Cámara Uruguaya del Arroz*

---

## La industria molinera

Entre la producción y la exportación, el arroz pasa por la **industria molinera** (molinos arroceros):

- **Molienda:** descascarado, blanqueado y pulido; subproductos (afrechillo de arroz, cáscara).
- Principales molinos en Treinta y Tres, Cerro Largo y otras zonas productoras.
- La **Cámara Uruguaya del Arroz (CUA)** agrupa a los principales actores de la cadena.
- Creciente inversión en **parboilizado** (arroz parboil) y otros procesos de valor agregado.

---

## Mercados de exportación

| Destino | Participación estimada |
|---------|----------------------|
| Brasil | ~30–40% del volumen exportado |
| Perú | ~10–15% |
| Senegal, Costa de Marfil, otros África | ~10–20% |
| Irak, Oriente Medio | ~5–10% |
| Otros América Latina | ~5–10% |
| Otros | Variable |

Uruguay exporta principalmente **arroz blanco** (grano largo, cristalino, de alta calidad). Se diferencia en el mercado internacional por su calidad culinaria, inocuidad y trazabilidad.

---

## Cooperativas arroceras

El sector arrocero tiene una larga tradición cooperativa:
- **COOPAR (Cooperativa Agraria Arrocera de Treinta y Tres):** una de las más importantes.
- **SAMAN (Sociedad Anónima Molinos Arroceros del Norte):** empresa mixta (productores + capital privado); uno de los mayores exportadores.
- Otras cooperativas departamentales.

---

## Impacto ambiental

- El arroz irrigado requiere grandes volúmenes de agua; gestión eficiente del agua es clave.
- Los arrozales proveen hábitat para aves acuáticas (patos, garzas, espátulas).
- Preocupaciones sobre escorrentía de nitrógeno, fósforo y agroquímicos hacia cursos de agua.
- Programas de buenas prácticas del sector para reducir el impacto sobre la Laguna Merín y sus tributarios.

---

## Palabras clave

arroz Uruguay producción exportación, cuenca arrocera este Uruguay Laguna Merín Treinta y Tres Cerro Largo, variedad INIA Tacuarí INIA Olimar El Paso 144 arroz Uruguay, sistema riego inundación taipas arroz Uruguay, INIA Treinta y Tres mejoramiento arroz Uruguay, productividad arroz Uruguay 7500 8500 kg hectárea, Cámara Uruguaya del Arroz CUA molinos molienda, exportaciones arroz Uruguay Brasil Perú África Irak, COOPAR SAMAN cooperativas arroceras Uruguay, arroz parboilizado valor agregado Uruguay
