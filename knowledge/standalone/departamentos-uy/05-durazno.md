# Departamento de Durazno

## Ficha técnica

| Parámetro | Valor |
|-----------|-------|
| **Nombre oficial** | Departamento de Durazno |
| **Capital** | Ciudad de Durazno |
| **Superficie** | 11.643 km² |
| **Población** | 62.011 hab. (Censo 2023) |
| **Densidad** | 5,3 hab./km² |
| **Gentilicio** | Durazniense |
| **Creación** | 1816 (decreto artiguista); consolidado en 1830 |
| **Ubicación** | Centro de Uruguay |
| **Límites** | Tacuarembó (norte), Cerro Largo (noreste), Florida (este), Lavalleja (sureste), Flores (sur), Soriano (suroeste), Río Negro (oeste) |
| **Rutas principales** | Ruta 5 (hacia Montevideo y Rivera), Ruta 14 |

---

## Historia

### Origen del nombre

El nombre "Durazno" proviene del árbol frutal durazno (melocotón, *Prunus persica*), abundante en la región en los tiempos de la fundación.

### Fundación

La ciudad de Durazno fue fundada el **25 de octubre de 1821** durante el dominio lusitano-brasileño de la Banda Oriental, bajo el nombre de **Villa de San Pedro del Durazno**.

### Siglo XIX–XX

Durazno fue un departamento de frontera en las guerras del siglo XIX. Su posición central en el país lo hizo punto de cruce de rutas ganaderas y militares. Durante el siglo XX se consolidó como centro ganadero y agrícola.

---

## Geografía

### Relieve

Durazno ocupa el **centro geográfico** del Uruguay. El relieve es ondulado, con cuchillas suaves (Cuchilla Grande al este, Cuchilla de Haedo al oeste). Altitudes entre 50 y 200 m s.n.m.

### Hidrografía

| Curso de agua | Descripción |
|--------------|-------------|
| **Río Negro** | Atraviesa el departamento de noreste a suroeste; el embalse del **Paso del Palmar** (represa Palmar) y el embalse de **Rincón del Bonete** (el mayor del Uruguay, ~1.000 km²) están en parte en Durazno |
| **Río Yi** | Nace en el departamento; afluente del Río Negro |
| **Arroyo Grande** | Afluente interno |

### Clima

Clima **templado húmedo** (Cfa). Temperatura media: ~17–18 °C. Precipitaciones: ~1.200–1.300 mm/año.

---

## Naturaleza

### Flora y fauna

Pastizales naturales dominantes; monte ribereño en las orillas del Río Negro y el Yi. Fauna de pradera: venado de campo, zorro gris, ñandú, mulita, armadillo.

### Lago Artificial de Rincón del Bonete

El **Embalse de Rincón del Bonete** (~1.000 km²) es el mayor lago artificial de Uruguay. Ofrece ecosistemas acuáticos únicos con aves acuáticas, peces y paisajes lacustres.

---

## Economía

### Ganadería

La ganadería extensiva de bovinos y ovinos es la actividad económica principal. Durazno tiene importantes frigoríficos y plantas de procesamiento cárnico.

### Agricultura

Soja, girasol y trigo en rotación con ganadería. El arroz se cultiva en zonas bajas.

### Energía hidroeléctrica

Las represas del Río Negro (**Rincón del Bonete**, **Baygorria** y **Palmar**) generan energía hidroeléctrica para la red nacional de UTE. La represa de Rincón del Bonete (inaugurada en **1945**) fue la primera gran represa hidroeléctrica de Uruguay.

---

## Educación

| Institución | Descripción |
|-------------|-------------|
| Escuelas del CEIP | Primaria pública |
| Liceos del CES | Educación media |
| UTU — Durazno | Formación técnica |
| Sede de UdelaR en Durazno [VERIFICAR oferta] | Educación universitaria |

---

## Salud

| Centro | Descripción |
|--------|-------------|
| **Hospital de Durazno** | Hospital público departamental (ASSE) |
| Mutualistas locales | Cobertura privada |

---

## Arquitectura y patrimonio

| Edificio / Sitio | Descripción |
|-----------------|-------------|
| **Plaza Batlle de Durazno** | Centro histórico de la capital |
| **Catedral de Durazno** | Patrimonio religioso |
| **Museo Histórico de Durazno** | Historia regional |

---

## Cultura

### Fiestas y tradiciones

| Evento | Descripción |
|--------|-------------|
| **Festival de Folclore de Durazno** | Importante festival nacional de música folclórica y gauchesca; se realiza anualmente en Durazno |
| **Semana Criolla** | Actividades gauchescas; doma |
| **Festival País** [VERIFICAR nombre] | — |

El **Festival de Folclore de Durazno** es uno de los festivales de música criolla y folclórica más importantes del Uruguay.

---

## Deportes

| Club | Descripción |
|------|-------------|
| **Club Atlético River Plate** (Durazno) | Fútbol departamental |
| Liga Departamental de Fútbol | Organismo local |

---

## Turismo

| Atractivo | Descripción |
|-----------|-------------|
| **Lago Rincón del Bonete** | Pesca deportiva; turismo lacustre; paisajes |
| **Parque Rodó de Durazno** | Parque urbano junto al Río Yi |
| **Termas del Arapey** (en Salto, accesibles desde Durazno) | — |

---

## Infraestructura

| Área | Descripción |
|------|-------------|
| **Ruta 5** | Principal arteria; conecta Montevideo con el norte (Rivera) pasando por Durazno |
| **Ruta 14** | Conexión este-oeste del centro del país |
| **Aeropuerto de Durazno** | Aeródromo regional [VERIFICAR operatividad] |

---

## Instituciones

| Institución | Descripción |
|-------------|-------------|
| **Intendencia de Durazno** | Gobierno departamental |
| **Junta Departamental** | 31 ediles |
| **Municipio de Durazno** | Gobierno municipal capital |
| **Municipio de Sarandí del Yi** | Segunda localidad del departamento |

---

## Personalidades destacadas

| Nombre | Área | Aporte |
|--------|------|--------|
| [VERIFICAR personalidades destacadas de Durazno] | — | — |

---

## Estadísticas

| Indicador | Valor | Fuente |
|-----------|-------|--------|
| Superficie | 11.643 km² | INE |
| Población (Censo 2023) | 62.011 hab. | INE |
| Densidad | 5,3 hab./km² | INE |

---

## Curiosidades

- Durazno es considerado el **centro geográfico** aproximado del Uruguay.
- La represa de **Rincón del Bonete** (1945) fue la **primera gran obra hidroeléctrica** del Uruguay y uno de los mayores proyectos de infraestructura del siglo XX.
- El **Festival de Folclore de Durazno** reúne cada año a decenas de miles de personas y es uno de los más importantes del folclore rioplatense.

---

## Cronología

| Año | Hito |
|-----|------|
| 1821 | Fundación de Villa de San Pedro del Durazno |
| 1830 | Organización formal del departamento |
| 1945 | Inauguración de la represa de Rincón del Bonete |
| 1960 | Inauguración de la represa de Baygorria |
| 1982 | Inauguración de la represa de Palmar |
| 2009 | Ley 18.567; creación de municipios |

---

## Palabras clave

departamento Durazno Uruguay, capital Durazno ciudad, represa Rincón del Bonete Uruguay, Río Negro Uruguay represas hidroeléctricas, Festival Folclore Durazno Uruguay, centro geográfico Uruguay Durazno, ganadería Durazno Uruguay, gentilicio durazniense, Intendencia Durazno, Ruta 5 Uruguay Durazno
