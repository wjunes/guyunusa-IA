# Organismos Vinculados al Sistema Educativo y Cultural Uruguayo

## Descripción

Este archivo recoge las **fichas institucionales** de los principales organismos vinculados al Ministerio de Educación y Cultura (MEC) y al Sistema Nacional de Educación Pública (SNEP) de Uruguay. Incluye entes autónomos, servicios descentralizados, institutos y organismos especializados que integran el ecosistema educativo, cultural y científico uruguayo.

## ANEP — Administración Nacional de Educación Pública

| Campo | Información |
|-------|-------------|
| Sigla | ANEP |
| Tipo | Ente autónomo del Estado |
| Base legal | Ley 15.739 (1985); Ley 18.437 (2008) |
| Sede | Florencio Sánchez 1021, Montevideo |
| Sitio web | www.anep.edu.uy |
| Misión | Gestionar la educación pública inicial, primaria, media, técnica y la formación docente |
| Subsistemas | DGEIP, DGES, DGETP (UTU), CFE |
| Gobierno | CODICEN (5 consejeros designados por el PE con venia del Senado) |

## CODICEN — Consejo Directivo Central

| Campo | Información |
|-------|-------------|
| Sigla | CODICEN |
| Tipo | Órgano de gobierno de la ANEP |
| Integración | Presidente + 4 Consejeros |
| Designación | Poder Ejecutivo con venia del Senado |
| Período | 5 años |
| Funciones | Conducción general de la ANEP; presupuesto; coordinación de subsistemas |

## DGEIP — Dirección General de Educación Inicial y Primaria

| Campo | Información |
|-------|-------------|
| Sigla | DGEIP |
| Cobertura | Educación inicial (3–5 años) y primaria (6–11 años) |
| Centros | Más de 2.400 escuelas en todo el país |
| Dependencia | CODICEN/ANEP |
| Gobierno | Consejo de 3 integrantes |
| Sitio web | www.dgeip.edu.uy |

## DGES — Dirección General de Educación Secundaria

| Campo | Información |
|-------|-------------|
| Sigla | DGES |
| Cobertura | Ciclo básico y bachillerato en liceos públicos |
| Centros | Más de 300 liceos |
| Dependencia | CODICEN/ANEP |
| Gobierno | Consejo de 3 integrantes |
| Sitio web | www.ces.edu.uy |

## DGETP / UTU — Dirección General de Educación Técnico-Profesional

| Campo | Información |
|-------|-------------|
| Sigla | DGETP; nombre popular: UTU |
| Cobertura | Educación técnica, bachilleratos tecnológicos, tecnicaturas |
| Centros | Más de 130 |
| Dependencia | CODICEN/ANEP |
| Gobierno | Consejo de 3 integrantes |
| Historia | Fundada como Escuela de Artes y Oficios (1878); UTU desde 1942 |
| Sitio web | www.utec.edu.uy (UTEC) / www.utu.edu.uy |

## CFE — Consejo de Formación en Educación

| Campo | Información |
|-------|-------------|
| Sigla | CFE |
| Cobertura | Formación inicial y continua de docentes |
| Institutos | IPA, institutos magisteriales, CERP (6), IPES |
| Dependencia | CODICEN/ANEP |
| Base legal | Ley 18.437 (2008) |
| Debate | IUDE: universitarización pendiente de definición |
| Sitio web | www.cfe.edu.uy |

## Udelar — Universidad de la República

| Campo | Información |
|-------|-------------|
| Nombre completo | Universidad de la República |
| Sigla | Udelar |
| Fundación | 1833 |
| Tipo | Ente autónomo; cogobernada |
| Matrícula | ~160.000 estudiantes activos (2026) |
| Sede central | Montevideo (Av. 18 de Julio 1968) |
| Centros regionales | CENUR Litoral Norte, Noreste, Este, Durazno |
| Sitio web | www.udelar.edu.uy |

## UTEC — Universidad Tecnológica del Uruguay

| Campo | Información |
|-------|-------------|
| Nombre completo | Universidad Tecnológica del Uruguay |
| Sigla | UTEC |
| Fundación | Ley 19.043 (2012) |
| Tipo | Persona jurídica de derecho público; vinculada al MEC |
| Sedes | 12 (incluyendo Minas, 2026) |
| Sitio web | www.utec.edu.uy |
| Misión | Universidad tecnológica descentralizada; interior del país |

## Plan Ceibal

| Campo | Información |
|-------|-------------|
| Nombre completo | Centro Ceibal para el Apoyo Tecnológico a la Educación Pública |
| Creación | Decreto 144/007 (2007) |
| Misión | Tecnología educativa; dispositivos, conectividad y plataformas digitales para el sistema educativo |
| Cobertura | Primaria completa (desde 2009); secundaria desde 2010 |
| Sitio web | www.ceibal.edu.uy |

## INEEd — Instituto Nacional de Evaluación Educativa

| Campo | Información |
|-------|-------------|
| Nombre completo | Instituto Nacional de Evaluación Educativa |
| Sigla | INEEd |
| Fundación | Ley 18.437 (2008); funcionamiento desde 2012 |
| Tipo | Organismo independiente |
| Misión | Evaluación externa del sistema educativo uruguayo |
| Actividades | Pruebas ARISTAS; participación en PISA, ERCE; informes sobre el estado de la educación |
| Sitio web | www.ineed.edu.uy |

## ANII — Agencia Nacional de Investigación e Innovación

| Campo | Información |
|-------|-------------|
| Nombre completo | Agencia Nacional de Investigación e Innovación |
| Sigla | ANII |
| Fundación | Ley 18.084 (2006) |
| Tipo | Persona jurídica de derecho público no estatal |
| Misión | Fomento de la investigación científica y la innovación en Uruguay |
| Instrumentos | SNII, becas de posgrado, fondos de I+D, emprendedurismo |
| Sitio web | www.anii.org.uy |

## IIBCE — Instituto de Investigaciones Biológicas Clemente Estable

| Campo | Información |
|-------|-------------|
| Nombre completo | Instituto de Investigaciones Biológicas Clemente Estable |
| Sigla | IIBCE |
| Fundación | 1927 |
| Dependencia | MEC / DINACT |
| Misión | Investigación básica y aplicada en ciencias biológicas |
| Áreas | Neurociencias, genética, microbiología, biología celular |
| Sitio web | www.iibce.edu.uy |

## Biblioteca Nacional

| Campo | Información |
|-------|-------------|
| Fundación | 1816 |
| Dependencia | MEC |
| Misión | Custodia y difusión del patrimonio bibliográfico nacional; depósito legal |
| Sede | Av. 18 de Julio 1790, Montevideo |
| Colección | Más de 1 millón de volúmenes |

## Archivo General de la Nación (AGN)

| Campo | Información |
|-------|-------------|
| Sigla | AGN |
| Dependencia | MEC |
| Misión | Custodia y organización del acervo documental histórico del Estado |
| Fondos | Período colonial (1724) hasta presente |

## SODRE — Servicio Oficial de Difusión, Radiotelevisión y Espectáculos

| Campo | Información |
|-------|-------------|
| Sigla | SODRE |
| Fundación | 1929 |
| Tipo | Ente autónomo vinculado al MEC |
| Funciones | Televisión pública (Canal 5), radios públicas, ópera, ballet, orquesta sinfónica |
| Sede | Auditorio Nacional del SODRE, Montevideo |
| Sitio web | www.sodre.gub.uy |

## INEFOP — Instituto Nacional de Empleo y Formación Profesional

| Campo | Información |
|-------|-------------|
| Sigla | INEFOP |
| Tipo | Entidad pública no estatal (tripartita: gobierno, empleadores, trabajadores) |
| Misión | Formación laboral y profesional continua para adultos en el mercado de trabajo |
| Articulación | UTU/DGETP; ANEP; MTSS |
| Sitio web | www.inefop.org.uy |

## Palabras clave

organismos sistema educativo Uruguay, ANEP Uruguay fichas, CODICEN Uruguay, DGEIP DGES DGETP CFE, Udelar Uruguay, UTEC Uruguay, Plan Ceibal Uruguay, INEEd Uruguay, ANII Uruguay, IIBCE Uruguay, Biblioteca Nacional Uruguay, AGN Archivo General Nación Uruguay, SODRE Uruguay, INEFOP Uruguay, MEC Uruguay organismos vinculados, sistema educativo público Uruguay instituciones
