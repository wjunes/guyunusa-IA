# Uruguay y la Antártida

---

## La Antártida como dimensión oceánica de Uruguay

La relación de Uruguay con la Antártida es una dimensión estratégica de su política exterior y científica que tiene una directa vinculación con sus intereses marítimos y oceanográficos. Las aguas del Océano Austral que rodean el continente antártico se conectan directamente con el Atlántico Sudoccidental y, en consecuencia, con la plataforma continental y la Zona Económica Exclusiva de Uruguay. La dinámica oceanográfica del Océano Austral —en particular, la Corriente Circumpolar Antártica— determina en parte las características de las masas de agua que bañan las costas uruguayas.

---

## Historia de la vinculación de Uruguay con la Antártida

| Hito | Año |
|------|-----|
| Primeras menciones de interés antártico en Uruguay [VERIFICAR] | Mediados del siglo XX |
| Uruguay firma el Tratado Antártico | 1980 |
| Creación del IAU (Instituto Antártico Uruguayo) | 1968 [VERIFICAR si es anterior a la adhesión al Tratado] |
| Primera expedición antártica uruguaya [VERIFICAR] | [VERIFICAR] |
| Instalación de la Base Científica Antártica Artigas (BCAA) en Isla Rey Jorge | 11 de diciembre de 1984 |
| Uruguay, Parte Consultiva del Tratado Antártico | 1985 |
| Uruguay ratifica el Protocolo de Madrid (Protocolo al Tratado Antártico sobre Protección del Medio Ambiente) | [VERIFICAR año] |

---

## El Tratado Antártico y el Sistema del Tratado Antártico (STA)

El **Tratado Antártico** fue firmado en Washington el **1 de diciembre de 1959** y entró en vigor en 1961. Sus disposiciones principales incluyen:

- La Antártida se utilizará exclusivamente con fines pacíficos.
- Se prohíbe toda actividad militar, incluyendo ensayos de armas.
- Se garantiza la libertad de investigación científica.
- Se fomenta la cooperación internacional.
- Las reclamaciones territoriales quedan congeladas durante la vigencia del Tratado.

Uruguay, como **Parte Consultiva**, tiene derecho a voto en las Reuniones Consultivas del Tratado Antártico (RCTA), las cuales se celebran anualmente y toman decisiones sobre la gobernanza del continente.

### El Protocolo de Madrid (1991)

El **Protocolo al Tratado Antártico sobre Protección del Medio Ambiente** (Protocolo de Madrid) establece la Antártida como "reserva natural, consagrada a la paz y a la ciencia" y prohíbe las actividades minerales (salvo con fines científicos). Uruguay es Parte de este Protocolo.

---

## Base Científica Antártica Artigas (BCAA)

La **BCAA** es la instalación permanente de Uruguay en la Antártida. Ubicada en la **bahía Maxwell, Isla Rey Jorge** (Shetland del Sur), opera de manera continua desde 1984:

| Parámetro | Descripción |
|-----------|-------------|
| Localización | Bahía Maxwell, Isla Rey Jorge (Isla 25 de Mayo para Argentina), Shetlands del Sur |
| Coordenadas aproximadas | ~62° 11' S, 58° 54' O |
| Operación | Permanente (abierta todo el año) |
| Superficie construida | [VERIFICAR] |
| Capacidad de alojamiento | [VERIFICAR número de personas] |
| Personal de invierno | [VERIFICAR número] |
| Personal de verano (campaña) | [VERIFICAR número] |

### Actividades en la BCAA

- Investigación científica multidisciplinar (ver archivo 12 sobre el IAU).
- Observaciones meteorológicas sistemáticas transmitidas a la red mundial de la OMM.
- Monitoreo ambiental.
- Apoyo logístico a las campañas de investigación.

---

## Intereses uruguayos en la Antártida

### Científicos

- Estudio de los ecosistemas antárticos y sus respuestas al cambio climático.
- Investigación de la dinámica oceánica del Océano Austral y su interacción con el Atlántico Sudoccidental.
- Monitoreo de la criosfera (hielos marinos y continentales) y su relación con el nivel del mar.
- Biología y ecología de las especies antárticas (pingüinos, focas, krill, peces antárticos).

### Estratégicos y de política exterior

- La condición de Parte Consultiva del Tratado Antártico otorga a Uruguay voz y voto en la gobernanza de un continente de enorme importancia global.
- El Tratado Antártico vincula a Uruguay con una alianza de países que incluye a las principales potencias mundiales.
- La presencia permanente en la Antártida es un elemento de la proyección internacional de Uruguay.

### Económicos (potenciales)

- Los recursos vivos del Océano Austral —en particular el krill y los peces antárticos— son objeto de gestión en el marco de la CCAMLR, en la que Uruguay participa.
- El debate sobre recursos minerales antárticos (actualmente prohibidos por el Protocolo de Madrid) es relevante para el futuro a largo plazo.

---

## CCAMLR y los recursos vivos del Océano Austral

La **Comisión para la Conservación de los Recursos Vivos Marinos Antárticos (CCAMLR)**, establecida por la Convención de Canberra (1980), gestiona los recursos vivos del Océano Austral al sur de la Convergencia Antártica. Uruguay es miembro de la CCAMLR y participa en sus reuniones anuales.

Las pesquerías más relevantes bajo la gestión de la CCAMLR incluyen:

| Especie | Nombre científico | Relevancia para Uruguay |
|---------|-------------------|------------------------|
| Krill antártico | *Euphausia superba* | Recurso de alto interés para diversas industrias; Uruguay [VERIFICAR si tiene buques krileros] |
| Merluza negra | *Dissostichus eleginoides* | Especie de alto valor comercial; pesquería en las Islas Georgias del Sur y otras zonas australes |
| Bacalao de profundidad antártico | *Dissostichus mawsoni* | Alta Mar antártica; pesca regulada por CCAMLR |

---

## Logística antártica uruguaya

El acceso y el suministro de la BCAA se realizan mediante:

- **Fuerza Aérea Uruguaya (FAU):** vuelos regulares a la Isla Rey Jorge durante la campaña de verano austral, generalmente desde Montevideo con escala en Punta Arenas (Chile).
- **Armada Nacional:** transporte marítimo de cargas pesadas, combustible y personal; ocasionalmente buques con capacidad de investigación oceanográfica.

---

## Conexión oceanográfica: Antártida y Uruguay

El Océano Austral y la Antártida tienen una profunda influencia sobre la oceanografía de las aguas uruguayas:

- La **Corriente de Malvinas** es una rama de la **Corriente Circumpolar Antártica** y es la que trae las aguas frías y ricas en nutrientes al margen continental uruguayo.
- El **Agua Antártica Intermedia (AAIW)** [VERIFICAR presencia en plataforma uruguaya] y otras masas de agua de origen antártico están presentes en las capas intermedias y profundas del Atlántico Sudoccidental frente a Uruguay.
- Los cambios en la criosfera antártica —derretimiento de hielos, variabilidad en la extensión del hielo marino— tienen impactos directos sobre la circulación oceánica global y, en última instancia, sobre el nivel del mar y la oceanografía de las aguas uruguayas.

---

## Palabras clave

Uruguay Antártida BCAA Base Científica Artigas Isla Rey Jorge 1984, Tratado Antártico Uruguay 1980 Parte Consultiva 1985 Sistema Tratado, IAU Instituto Antártico Uruguayo Ministerio Relaciones Exteriores, CCAMLR Uruguay recursos vivos marinos antárticos merluza negra krill, Corriente de Malvinas Corriente Circumpolar Antártica Uruguay aguas frías, Protocolo de Madrid 1991 Uruguay protección medio ambiente antártico, logística antártica Uruguay FAU Armada Punta Arenas vuelos, cambio climático criosfera antártica nivel mar Uruguay, investigación científica BCAA meteorología biología marina antártica, oceanografía Océano Austral Uruguay conexión masas de agua
