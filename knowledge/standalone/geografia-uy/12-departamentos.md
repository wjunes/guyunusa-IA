# Fichas Geográficas de los Departamentos del Uruguay

## Descripción general

Uruguay se divide en **19 departamentos**. Este archivo presenta fichas geográficas resumidas de cada departamento, incluyendo su ubicación, superficie, capital, características físicas, economía y datos relevantes para la recuperación de información geográfica.

Para información ampliada de cada departamento, véase el dominio **departamentos-uy** de la BNC-UY.

---

## 01 — Artigas

| Parámetro | Dato |
|-----------|------|
| **Capital** | Artigas (fundada 1852) |
| **Superficie** | ~11.928 km² |
| **Límites** | Brasil (norte y este), Río Uruguay (oeste), Rivera y Salto (sur) |
| **Relieve** | Cuchilla de Haedo (sur); basaltos y areniscas; llanuras del Río Cuareim |
| **Hidrografía** | Río Cuareim (frontera), Río Uruguay (oeste), Río Arapey (sur) |
| **Economía** | Ganadería; arroz (Bella Unión); caña de azúcar (Bella Unión, ALUR); ágatas y amatistas |
| **Otros hitos** | Ágatas y amatistas de Artigas (líder mundial); Bella Unión (extremo noroeste de Uruguay) |

---

## 02 — Canelones

| Parámetro | Dato |
|-----------|------|
| **Capital** | Canelones |
| **Superficie** | ~4.536 km² |
| **Límites** | Río de la Plata (sur), Montevideo (sur-oeste), San José (oeste), Florida (norte), Maldonado (este) |
| **Relieve** | Llanura costera; ondulaciones suaves del sur; cuchillas menores al norte |
| **Hidrografía** | Río Santa Lucía (oeste), Arroyo Solís Grande (este), múltiples arroyos |
| **Economía** | Horticultura (principal productor hortícola); vitivinicultura (Tannat, Juanicó, Establecimiento Juanicó); lechería; industria; Aeropuerto Internacional Carrasco |
| **Otros hitos** | Aeropuerto Internacional de Carrasco (MVD); Planta OSE Aguas Corrientes (cuenca Santa Lucía); segundo departamento más poblado (~570.000 hab.) |

---

## 03 — Cerro Largo

| Parámetro | Dato |
|-----------|------|
| **Capital** | Melo (fundada 1795) |
| **Superficie** | ~13.648 km² |
| **Límites** | Brasil (norte), Rivera (noroeste), Tacuarembó (oeste), Durazno (suroeste), Treinta y Tres (sur), Rocha (sureste) |
| **Relieve** | Cuchilla Grande (sur-este); Cuchilla de Río Branco; ondulaciones suaves en el centro |
| **Hidrografía** | Río Yaguarón (frontera norte), Río Tacuarí, Laguna Merín (este) |
| **Economía** | Ganadería; arroz (cuenca del Tacuarí); minería (granito ornamental); frontera con Brasil |
| **Otros hitos** | Ciudad fronteriza Rio Branco–Jaguarão; cuenca arrocera del Tacuarí |

---

## 04 — Colonia

| Parámetro | Dato |
|-----------|------|
| **Capital** | Colonia del Sacramento (fundada 1680) |
| **Superficie** | ~6.106 km² |
| **Límites** | Río de la Plata (sur y oeste), Argentina (frente fluvial), San José (este), Soriano (norte) |
| **Relieve** | Llanura costera; suave ondulado |
| **Hidrografía** | Río de la Plata; Río San Juan; Río Rosario |
| **Economía** | Lechería; agricultura (trigo, soja); turismo histórico; colonias agrícolas (valdenses, suizo-alemanes) |
| **Otros hitos** | Barrio Histórico de Colonia del Sacramento (UNESCO 1995); ferrys a Buenos Aires; Nueva Helvecia (colonia suizo-alemana, 1862); colonias valdenses (1856) |

---

## 05 — Durazno

| Parámetro | Dato |
|-----------|------|
| **Capital** | Durazno |
| **Superficie** | ~11.643 km² |
| **Límites** | Tacuarembó (norte), Cerro Largo y Treinta y Tres (este), Flores y Florida (sur), Río Negro (oeste) |
| **Relieve** | Centro geográfico de Uruguay; suave ondulado; llanuras del Río Negro y el Río Yi |
| **Hidrografía** | Río Negro y sus embalses (Bonete, Baygorria); Río Yi |
| **Economía** | Ganadería extensiva; energía hidroeléctrica (represas); Festival de Folclore |
| **Otros hitos** | Centro geográfico aproximado del Uruguay; Represa Rincón del Bonete (1945, ~160 MW) |

---

## 06 — Flores

| Parámetro | Dato |
|-----------|------|
| **Capital** | Trinidad (fundada 1805) |
| **Superficie** | ~5.144 km² |
| **Límites** | Soriano (oeste), Río Negro (norte), Durazno (noreste), Florida (este), San José (sur) |
| **Relieve** | Suave ondulado; llanuras del Río Negro; cuchillas menores |
| **Hidrografía** | Río Yi (este), Río Negro (norte) |
| **Economía** | Ganadería extensiva |
| **Otros hitos** | Departamento más nuevo (creado 16/10/1901); segundo más pequeño en superficie; menos poblado (~25.000 hab.) |

---

## 07 — Florida

| Parámetro | Dato |
|-----------|------|
| **Capital** | Florida (fundada 1809) |
| **Superficie** | ~10.417 km² |
| **Límites** | Tacuarembó (norte), Durazno y Cerro Largo (noreste), Lavalleja (este), Canelones (sur), San José (suroeste), Flores y Durazno (oeste) |
| **Relieve** | Cuchilla Grande (sureste); llanuras al oeste |
| **Hidrografía** | Río Santa Lucía (nace aquí; Embalse Paso Severino); Río Yí |
| **Economía** | Ganadería; lechería; agricultura |
| **Otros hitos** | Declaración de la Independencia de Uruguay (25/08/1825, Sala de Representantes de Florida); Embalse Paso Severino (agua potable Montevideo) |

---

## 08 — Lavalleja

| Parámetro | Dato |
|-----------|------|
| **Capital** | Minas (fundada 1783) |
| **Superficie** | ~10.016 km² |
| **Límites** | Treinta y Tres (norte), Rocha (este), Maldonado (sur), Canelones y Florida (oeste) |
| **Relieve** | Cuchilla Grande; sierras; **Cerro Catedral (~514 m, punto más alto de Uruguay)**; Cerro Arequita |
| **Hidrografía** | Río Cebollati (naciente), Arroyo Grande, Arroyo Cuñapirú |
| **Economía** | Ganadería; minería (granito, mármol; canteras de Minas); agua mineral (Nativa); turismo rural |
| **Otros hitos** | Punto más alto de Uruguay (Cerro Catedral); Cerro Arequita (zona de escalada); Peregrinación al Verdún (santuario religioso); agua mineral Nativa |

---

## 09 — Maldonado

| Parámetro | Dato |
|-----------|------|
| **Capital** | Maldonado (fundada 1755) |
| **Superficie** | ~4.793 km² |
| **Límites** | Rocha (este), Lavalleja (norte), Canelones (oeste), Océano Atlántico y Río de la Plata (sur) |
| **Relieve** | Sierras (Aiguá, Carapé; cerros: Catedral en Lavalleja/límite; Las Ánimas ~501 m; Pan de Azúcar ~423 m); llanura costera |
| **Hidrografía** | Laguna del Sauce (agua potable); Laguna Garzón; Arroyo Solís Grande (límite oeste); Río Santa Lucía Chico |
| **Economía** | Turismo de playa (Punta del Este, principal resort de América del Sur); ganadería; construcción; pesca |
| **Otros hitos** | Punta del Este (turismo internacional); Casapueblo (Carlos Páez Vilaró); Isla de Lobos (SNAP, ~200.000 lobos marinos); "La Mano en la Arena" (Mario Irarrázabal, 1982); CURE (UdelaR) |

---

## 10 — Montevideo

| Parámetro | Dato |
|-----------|------|
| **Capital** | Montevideo |
| **Superficie** | ~530 km² (más pequeño de Uruguay) |
| **Límites** | Canelones (norte), Río de la Plata (sur, este y oeste) |
| **Relieve** | Llanura costera; Cerro de Montevideo (~139 m) |
| **Hidrografía** | Río de la Plata; Arroyo Carrasco (este), Arroyo Pantanoso (oeste), Arroyo Miguelete (centro) |
| **Economía** | Principal nodo económico del país: finanzas, comercio, industria, servicios, Puerto de Montevideo |
| **Otros hitos** | Capital nacional; ~1.350.000 hab. (~40% de la población del país); Rambla (~22 km); Estadio Centenario (Mundial 1930); Puerto de Montevideo; Aeropuerto de Carrasco (en Canelones pero sirve a Montevideo); Ciudad Vieja (patrimonio histórico); Candombe UNESCO 2009 |

---

## 11 — Paysandú

| Parámetro | Dato |
|-----------|------|
| **Capital** | Paysandú |
| **Superficie** | ~13.922 km² |
| **Límites** | Salto (norte), Tacuarembó y Rivera (este), Río Negro y Soriano (sur), Río Uruguay/Argentina (oeste) |
| **Relieve** | Cuchilla de Haedo (este); llanuras del litoral; cuestas basálticas |
| **Hidrografía** | Río Uruguay (oeste); Río Queguay; Termas del Guaviyú (Acuífero Guaraní) |
| **Economía** | Ganadería; citricultura; Maltería Uruguay (cebada maltera, Paysandú); forestación; turismo termal |
| **Otros hitos** | Sitio de Paysandú (1864–65; Leandro Gómez); Puente Internacional Paysandú–Colón (Argentina); Julio Sosa ("el Varón del Tango", nacido en San José de Mayo pero vinculado al litoral); Termas del Guaviyú |

---

## 12 — Río Negro

| Parámetro | Dato |
|-----------|------|
| **Capital** | Fray Bentos |
| **Superficie** | ~9.282 km² |
| **Límites** | Paysandú (norte), Soriano (este), Río Uruguay/Argentina (oeste) |
| **Relieve** | Llanura del litoral; cuchillas menores |
| **Hidrografía** | Río Uruguay; Río Negro (norte); Represa El Palmar (1982) |
| **Economía** | Celulosa (UPM Fray Bentos); ganadería; lechería; forestación |
| **Otros hitos** | Frigorífico Anglo de Fray Bentos (UNESCO 2015); UPM Fray Bentos (celulosa, ~880.000 t/año); Puente General San Martín (Fray Bentos–Puerto Unzué, AR); Esteros de Farrapos (Ramsar, SNAP); colonia San Javier (inmigrantes rusos, 1913) |

---

## 13 — Rivera

| Parámetro | Dato |
|-----------|------|
| **Capital** | Rivera |
| **Superficie** | ~9.370 km² |
| **Límites** | Brasil (norte y este), Artigas (noroeste), Salto (oeste), Tacuarembó (sur) |
| **Relieve** | Cuchilla de Haedo (oeste), Cuchilla de Santana (este) |
| **Hidrografía** | Río Cuareim (noroeste), Río Tacuarembó (sur) |
| **Economía** | Ganadería; forestación (eucaliptos); turismo de compras; Zona Franca de Rivera |
| **Otros hitos** | Rivera–Santana do Livramento: ciudad binacional Uruguay-Brasil sin barreras físicas; Portuñol/DPU (lengua de frontera); Minas de Corrales (oro); Zona Franca de Rivera |

---

## 14 — Rocha

| Parámetro | Dato |
|-----------|------|
| **Capital** | Rocha |
| **Superficie** | ~10.551 km² |
| **Límites** | Brasil (este), Cerro Largo (norte), Treinta y Tres (noroeste), Lavalleja y Maldonado (oeste), Océano Atlántico (sur) |
| **Relieve** | Planicie costera; dunas (Cabo Polonio); suave ondulado |
| **Hidrografía** | Laguna Merín (este), Laguna Negra, Laguna de Rocha, Laguna Castillos, Río Cebollatí |
| **Economía** | Ganadería; arroz; turismo (playas no masificadas; Cabo Polonio, La Pedrera, Punta del Diablo); pesca; soja |
| **Otros hitos** | Bañados del Este (Ramsar 1984, primer sitio Ramsar de Uruguay); Cabo Polonio (SNAP, dunas, lobos marinos, faro); Palmar de Butiá (*Butia odorata*, VU; mayor concentración mundial); Fortaleza de San Miguel (1734); Barra del Chuy |

---

## 15 — Salto

| Parámetro | Dato |
|-----------|------|
| **Capital** | Salto (segunda ciudad interior de Uruguay) |
| **Superficie** | ~14.163 km² |
| **Límites** | Artigas (norte), Rivera (este), Paysandú (sur), Río Uruguay/Argentina (oeste) |
| **Relieve** | Cuchilla de Haedo (este); llanuras del litoral norte |
| **Hidrografía** | Río Uruguay; Río Arapey; Río Daymán; Embalse de Salto Grande |
| **Economía** | Energía hidroeléctrica (Salto Grande); citricultura; ganadería; turismo termal |
| **Otros hitos** | Represa de Salto Grande (CTM Uruguay-Argentina, ~1.890 MW, inaugurada 1979–1981); Termas del Daymán (mayor complejo termal del Uruguay); Puente Internacional Salto–Concordia; Luis Suárez (nacido en Salto, 24/01/1987) |

---

## 16 — San José

| Parámetro | Dato |
|-----------|------|
| **Capital** | San José de Mayo (fundada 1783) |
| **Superficie** | ~4.992 km² |
| **Límites** | Colonia (oeste), Soriano (noroeste), Flores (norte), Florida (noreste), Canelones (este), Río de la Plata (sur) |
| **Relieve** | Llanura costera sur; suave ondulado |
| **Hidrografía** | Río Santa Lucía; Río San José; Río de la Plata |
| **Economía** | Lechería (cuenca sur de CONAPROLE); horticultura; ganadería |
| **Otros hitos** | Cuenca lechera sur; Ciudad del Plata (expansión metropolitana de Montevideo); balnearios Kiyú y Playa Pascual |

---

## 17 — Soriano

| Parámetro | Dato |
|-----------|------|
| **Capital** | Mercedes |
| **Superficie** | ~9.008 km² |
| **Límites** | Río Uruguay/Argentina (oeste), Paysandú (norte), Río Negro (noroeste y este), Flores (este), San José y Colonia (sur) |
| **Relieve** | Llanura del litoral; ondulaciones suaves |
| **Hidrografía** | Río Uruguay; Río Negro; Río San Salvador |
| **Economía** | Ganadería; lechería; soja y agricultura; fruticultura |
| **Otros hitos** | Playa de la Agraciada: desembarco de los 33 Orientales (19/04/1825); Nuevo Berlín (colonia alemana) |

---

## 18 — Tacuarembó

| Parámetro | Dato |
|-----------|------|
| **Capital** | Tacuarembó (Villa San Fructuoso) |
| **Superficie** | ~15.438 km² (mayor departamento de Uruguay) |
| **Límites** | Rivera y Artigas (norte), Cerro Largo (este), Durazno y Flores (sur), Río Negro y Paysandú (oeste) |
| **Relieve** | Cuchilla de Haedo (oeste), Cuchilla de Santana (noreste); sierras; Valle del Lunarejo |
| **Hidrografía** | Río Tacuarembó, Río Negro (naciente), Río Cuareim (noroeste) |
| **Economía** | Ganadería extensiva sobre basalto; forestación; arándanos; turismo rural |
| **Otros hitos** | Departamento más extenso de Uruguay (~15.438 km²); Festival de Folclore de Tacuarembó (mayor festival gaucho del país); controversia sobre el nacimiento de Carlos Gardel; Valle del Lunarejo (quebrada con vegetación de Mata Atlántica); Estación experimental INIA Tacuarembó |

---

## 19 — Treinta y Tres

| Parámetro | Dato |
|-----------|------|
| **Capital** | Treinta y Tres |
| **Superficie** | ~9.529 km² |
| **Límites** | Cerro Largo (norte y noroeste), Lavalleja (oeste), Rocha (sur y suroeste) |
| **Relieve** | Planicie costera oriental; Quebrada de los Cuervos (sierras del sureste) |
| **Hidrografía** | Río Olimar, Río Cebollatí, Laguna Merín (este) |
| **Economía** | Arroz (principal productor nacional); ganadería |
| **Otros hitos** | Principal zona arrocera de Uruguay (cuenca del Olimar/Laguna Merín); Quebrada de los Cuervos (SNAP, tucán *Ramphastos toco*); Bañados del Este (Ramsar 1984); El Olimareño (cantautor José Luis Guerra); único departamento uruguayo con nombre numérico |

---

## Palabras clave

19 departamentos Uruguay fichas geográficas capital superficie, Tacuarembó mayor departamento Uruguay 15438 km², Montevideo menor departamento Uruguay 530 km² más poblado, Flores menos poblado Uruguay 25000 habitantes, Colonia del Sacramento Barrio Histórico UNESCO 1995, Rocha Bañados del Este Ramsar Cabo Polonio SNAP, Salto Represa Grande CTM termas Daymán, Maldonado Punta del Este turismo, Florida declaración independencia Uruguay 1825, Artigas ágatas amatistas, Fray Bentos UNESCO 2015 UPM celulosa, Rivera ciudad binacional Brasil Santana do Livramento
