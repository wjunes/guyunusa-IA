# Transparencia y Gobierno Abierto en Uruguay

## Introducción

Uruguay ha construido un marco institucional robusto en materia de **transparencia pública**, **acceso a la información** y **gobierno abierto**. Este marco combina una legislación avanzada, organismos especializados y compromisos internacionales que posicionan al país como referente en la región en integridad y apertura del Estado.

## Acceso a la Información Pública

### Ley 18.381/2008 (Acceso a la Información Pública)

La **Ley 18.381** de 17 de octubre de 2008 reconoció el **derecho de acceso a la información pública** como un derecho fundamental de todos los habitantes y ciudadanos de Uruguay. Principios:

- **Máxima divulgación**: toda información en poder del Estado es pública, salvo excepciones expresamente previstas.
- **Publicidad activa**: los organismos deben publicar proactivamente información sin necesidad de que sea solicitada.
- **Gratuidad**: el acceso a la información es gratuito.
- **Sin necesidad de justificación**: cualquier persona puede solicitar información sin explicar los motivos.

**Alcance**: aplica a todos los organismos del Estado uruguayo, incluyendo el Poder Ejecutivo, Legislativo, Judicial, entes autónomos, servicios descentralizados, empresas públicas y personas públicas no estatales.

**Excepciones**: la ley establece excepciones taxativas (información clasificada por razones de seguridad nacional, datos personales sensibles, secreto comercial o industrial, entre otros).

## Unidad de Acceso a la Información Pública (UAIP)

### Creación y marco legal

La **UAIP** fue creada por la **Ley 18.381/2008** (art. 19) y fortalecida por la **Ley 19.178/2013**, que le otorgó mayor autonomía. Hasta 2013 dependía de la Presidencia de la República; con la nueva ley pasó a funcionar como **órgano desconcentrado** con independencia técnica.

### Competencias

- Promover la transparencia de la función pública.
- Coordinar la política de acceso a la información.
- Recibir y resolver reclamos de ciudadanos cuyos pedidos de información fueron denegados.
- Asistir técnicamente a los organismos públicos en materia de acceso a la información.
- Controlar el cumplimiento de la Ley 18.381.
- Dictar resoluciones vinculantes sobre pedidos de acceso.

## Protección de Datos Personales

La **Ley 18.331/2008** (Protección de Datos Personales y Habeas Data) establece el marco para la protección de los datos personales de los ciudadanos. La **UAIP** actúa también como **Unidad Reguladora de Protección de Datos (URCDP)**, registrando las bases de datos con información personal y resolviendo reclamos.

Uruguay es reconocido internacionalmente por su legislación de protección de datos: la Unión Europea lo incluyó en la lista de países con nivel adecuado de protección (Decisión 2012/484/UE).

## Declaraciones Juradas de Bienes e Ingresos

### Junta de Transparencia y Ética Pública (JUTEP)

La **JUTEP** fue creada por la **Ley 17.060/1998** (Uso Indebido del Poder Público — Normas de Conducta en la Función Pública). Sus funciones:

- Recibir, archivar y controlar las **declaraciones juradas de bienes e ingresos** de los funcionarios públicos obligados (jerarcas, tomadores de decisiones con manejo de fondos públicos).
- Promover la ética e integridad pública.
- Asesorar a organismos sobre conflictos de interés.
- Recibir denuncias sobre conductas contrarias a la ética.

Todos los funcionarios con cargos de responsabilidad deben presentar declaración jurada al inicio, durante y al cese de sus funciones.

## Rendición de Cuentas

La **Rendición de Cuentas** es el mecanismo constitucional (art. 214–216 de la Constitución) por el cual el Poder Ejecutivo informa anualmente al Parlamento sobre el uso de los recursos públicos y solicita los ajustes al presupuesto vigente. Es el principal instrumento de **accountability vertical** del Estado uruguayo.

El **Portal de Transparencia Presupuestal** (transparencia.opp.gub.uy) publica en tiempo real la ejecución presupuestal del Estado, permitiendo a cualquier ciudadano seguir el gasto público.

## Gobierno Abierto y la OGP

Uruguay se integró a la **Alianza para el Gobierno Abierto (Open Government Partnership — OGP)** en 2011. La OGP es una iniciativa multilateral que promueve que los gobiernos sean más transparentes, participativos y responsables.

### Planes de Acción de Gobierno Abierto

Uruguay presenta **Planes de Acción bianuales** ante la OGP, con compromisos verificables en áreas como:
- Transparencia y datos abiertos.
- Participación ciudadana en políticas públicas.
- Integridad y lucha contra la corrupción.
- Gobierno digital y servicios públicos.

La sociedad civil participa activamente en la elaboración y monitoreo de los planes a través del **Espacio de Participación** coordinado por AGESIC y la UAIP.

## Participación Ciudadana

Uruguay cuenta con varios mecanismos de participación ciudadana en la gestión pública:

| Mecanismo | Descripción |
|---|---|
| **Plebiscito y referéndum** | Instrumentos constitucionales de democracia directa (arts. 79, 304, 331 Constitución) |
| **Iniciativa popular** | Los ciudadanos pueden presentar proyectos de ley con 25% de firmas del padrón |
| **Presupuesto participativo** | Mecanismo de algunas intendencias (IMM) para asignar recursos con participación vecinal |
| **Consultas públicas** | Organismos publican proyectos de normas para recibir comentarios de la ciudadanía |
| **Cabildos Abiertos** | Instancias de consulta pública convocadas por el Poder Ejecutivo u organismos |
| **Consejos consultivos** | Órganos multiactorales en políticas sectoriales (salud, educación, medioambiente) |

## Contratación pública y transparencia

El **SICP (Sistema de Información de Compras y Contrataciones del Estado)** (comprasestatales.gub.uy) publica en línea todos los procesos de compras y contrataciones del Estado uruguayo. La **ARCE (Agencia Reguladora de Compras Estatales)** regula los procedimientos de adquisición pública. Estos mecanismos garantizan competencia, publicidad y no discriminación en las compras del Estado.

## Palabras clave

transparencia Uruguay, acceso información pública Uruguay, Ley 18.381 Uruguay, UAIP Uruguay, JUTEP Uruguay, declaración jurada bienes funcionarios, protección datos personales Uruguay Ley 18.331, gobierno abierto Uruguay OGP, Open Government Partnership Uruguay, rendición de cuentas Uruguay, participación ciudadana Uruguay, plebiscito referéndum Uruguay, Portal Transparencia Presupuestal, ARCE compras estatales Uruguay, ética pública Uruguay
