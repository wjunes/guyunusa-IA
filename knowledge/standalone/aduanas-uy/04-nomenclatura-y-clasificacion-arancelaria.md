# Nomenclatura y clasificación arancelaria en Uruguay

## El Sistema Armonizado (SA)

La clasificación arancelaria de las mercancías en Uruguay —y en prácticamente todo el mundo— se basa en el Sistema Armonizado de Designación y Codificación de Mercancías (SA), administrado por la Organización Mundial de Aduanas (OMA). El SA fue creado por el Convenio Internacional del Sistema Armonizado, aprobado en Bruselas en 1983 y en vigor desde 1988.
...
La **NCM (8 dígitos)** es la nomenclatura oficial utilizada en Uruguay para las declaraciones aduaneras:

```
XXXX  XX  XX  XX
│     │   │   └── Subpartida nacional/MERCOSUR (2 dígitos)
│     │   └────── Subpartida SA (2 dígitos)
│     └────────── Partida SA (2 dígitos)
└──────────────── Capítulo SA (2 dígitos)
```

Ejemplo: `8471.30.10` — ordenadores portátiles.
...
- **21 Secciones** (agrupaciones temáticas amplias): animales vivos, productos vegetales, grasas y aceites, preparados alimenticios, productos minerales, química, plásticos, cuero, madera, papel, textiles, calzado, metales comunes, maquinaria, material eléctrico, transporte, óptica, armas, mercancías diversas, obras de arte.
- **97 Capítulos** (subdivisiones de las secciones).
- **Partidas** (4 dígitos).
...
La clasificación de las mercancías se determina aplicando las seis Reglas Generales de Interpretación (RGI) del SA en orden jerárquico:

**RGI 1**: la clasificación se determina por el texto de las partidas y las notas de sección y capítulo (principio de literalidad).

**RGI 2a**: los artículos incompletos o sin terminar se clasifican como el artículo completo o terminado si presentan el carácter esencial de este.

**RGI 2b**: las mezclas o combinaciones de materiales se clasifican conforme a la regla 3.

**RGI 3**: cuando una mercancía pueda clasificarse en dos o más partidas, se aplican tres criterios en orden: (a) partida más específica; (b) partida del material constitutivo predominante; (c) partida que tenga el número más alto en la nomenclatura.

**RGI 4**: las mercancías que no puedan clasificarse conforme a las reglas anteriores se clasifican en la partida correspondiente a los artículos más similares.

**RGI 5**: aplicable a estuches, cajas y embalajes.
...
La clasificación arancelaria de una mercancía determina:

1. **El derecho de importación (arancel)** aplicable.
2. **La tasa de IVA** aplicable en la importación (básica del 22%, mínima del 10%, o exención).
3. **Si el IMESI aplica** y a qué tasa (cigarrillos, bebidas alcohólicas, combustibles, vehículos).
4. **Los requisitos de licencias, permisos o controles** previos a la importación (controles del MGAP, MSP, MIEM, URSEA, etc.).
5. **Las reglas de origen** aplicables en el marco de los acuerdos comerciales.
6. **Las prohibiciones o restricciones** de importación (bienes sujetos a control; CITES para fauna silvestre).
...
nomenclatura arancelaria Uruguay, Sistema Armonizado SA OMA Uruguay, NCM Nomenclatura Común MERCOSUR Uruguay, NANDINA Uruguay, clasificación arancelaria Uruguay, posición arancelaria Uruguay, código NCM 8 dígitos Uruguay, Reglas Generales Interpretación SA, consultas clasificación arancelaria DNA Uruguay, Comité Técnico 1 MERCOSUR clasificación, arancel integrado Uruguay DNA, actualización SA OMA Uruguay
