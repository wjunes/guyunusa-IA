# ALADI y acuerdos bilaterales de Uruguay

## La ALADI como marco multilateral regional

La **ALADI** (Asociación Latinoamericana de Integración) es el mayor bloque de integración comercial de América Latina. Fue creada por el **Tratado de Montevideo de 1980** (firmado el 12 de agosto de 1980), que reemplazó a la ALALC (Asociación Latinoamericana de Libre Comercio, 1960). La ALADI tiene su sede en Montevideo, Uruguay.

**Países miembros**: Argentina, Bolivia, Brasil, Chile, Colombia, Cuba, Ecuador, México, Panamá (desde 2012), Paraguay, Perú, Uruguay y Venezuela.

**Principios**: la ALADI promueve la integración comercial mediante:
- Preferencias arancelarias regionales (PAR): reducción del AEC de cada miembro para el comercio con los demás.
- Acuerdos de Alcance Regional (AAR): entre todos los miembros.
- Acuerdos de Alcance Parcial (AAP): entre dos o más miembros.

## El MERCOSUR como ACE en el marco de la ALADI

El MERCOSUR está registrado en la ALADI como el **ACE-18** (Acuerdo de Complementación Económica n.º 18), firmado en 1991 entre Argentina, Brasil, Paraguay y Uruguay. Esta inscripción permite que el MERCOSUR opere dentro del marco jurídico de la ALADI.

## Principales acuerdos de Uruguay en el marco de la ALADI

### ACE-35: MERCOSUR-Chile

El **ACE-35** (firmado en 1996) estableció una zona de libre comercio gradual entre el MERCOSUR (incluido Uruguay) y Chile. La totalidad del universo arancelario fue desgravada progresivamente. Chile no es miembro del MERCOSUR pero mantiene este acuerdo de libre comercio con el bloque.

**Impacto para Uruguay**: el ACE-35 permite el acceso libre de aranceles a productos uruguayos en el mercado chileno y viceversa. Chile es un destino relevante para las exportaciones uruguayas de lácteos, arroz y manufacturas.

### ACE-36: MERCOSUR-Bolivia

El **ACE-36** establece preferencias arancelarias entre el MERCOSUR y Bolivia, con desgravación progresiva. Bolivia es un mercado de destino para el arroz uruguayo y otros productos agropecuarios.

### ACE-2: Argentina-Uruguay (antecedente histórico)

El ACE-2 bilateral entre Argentina y Uruguay fue uno de los primeros acuerdos de complementación económica de la ALADI entre ambos países, y antecedente del MERCOSUR. Fue absorbido por el ACE-18 (MERCOSUR) pero algunos de sus mecanismos bilaterales siguen vigentes.

### ACE-58 y ACE-59: MERCOSUR con Colombia, Ecuador, Venezuela y Perú

- **ACE-58** (MERCOSUR-Perú, 2005): libre comercio gradual entre Uruguay y Perú.
- **ACE-59** (MERCOSUR-Colombia, Ecuador, Venezuela, 2004): preferencias arancelarias con Colombia y Ecuador.

Estos acuerdos abren mercados para las exportaciones uruguayas de lácteos (Colombia, Ecuador, Perú), arroz y manufacturas.

### Acuerdos con México

Uruguay tiene un **Acuerdo de Complementación Económica** con México en el marco de la ALADI (ACE-5 Uruguay-México), que otorga preferencias arancelarias para una lista de productos uruguayos en el mercado mexicano. México es un mercado de interés para los servicios y las exportaciones agropecuarias uruguayas.

## Sistema Generalizado de Preferencias (SGP)

Uruguay se beneficia del **SGP** otorgado por países desarrollados (Unión Europea, Estados Unidos, Japón, Canadá, Suiza y otros) que conceden preferencias arancelarias a los países en desarrollo. El SGP reduce los aranceles pagaderos en esos mercados para una lista de productos uruguayos, aunque el alcance varía por país otorgante y por la graduación del beneficio (Uruguay, como país de renta media-alta, ha sido "graduado" del SGP de algunos otorgantes).

## Tratados bilaterales de inversión (BIT) y acuerdos complementarios

Uruguay ha firmado numerosos **Tratados Bilaterales de Inversión (TBI)** con países de interés inversor, que complementan los acuerdos comerciales al proteger la inversión extranjera. Estos acuerdos incluyen cláusulas de trato nacional, trato de nación más favorecida, protección contra la expropiación y mecanismos de solución de diferencias inversor-Estado (CIADI, CNUDMI).

## Posición de Uruguay en la OMC

Uruguay es miembro fundador de la **OMC** (Organización Mundial del Comercio, 1995) y fue signatario del GATT desde 1953. En el marco de la OMC:
- Presenta sus listas de concesiones arancelarias (aranceles consolidados).
- Participa en las negociaciones multilaterales (Ronda de Doha, Acuerdo de Facilitación del Comercio AFC, negociaciones sobre subsidios a la pesca).
- Ha utilizado el **Órgano de Solución de Diferencias (OSD)** de la OMC en disputas comerciales relevantes: en particular, Uruguay fue demandante (junto con Australia) en la disputa sobre el empaquetado neutro del tabaco, en el marco del caso WT/DS435 y WT/DS467 contra Australia (2012–2019), que la OMC resolvió a favor de Australia pero en sentido favorable a las políticas de salud pública, confirmando la soberanía regulatoria de los Estados.
- **Disputa con Philip Morris**: Uruguay fue también demandado por Philip Morris International ante el CIADI (caso PM v. Uruguay, 2010) por sus regulaciones de empaquetado del tabaco. El CIADI falló a favor de Uruguay en 2016, en un hito para la regulación del tabaco y el derecho de los Estados a legislar en salud pública.

## Negociaciones pendientes y futuro de los acuerdos bilaterales

- **TLC Uruguay-China**: Uruguay inició conversaciones bilaterales durante el gobierno de Lacalle Pou, bajo tensión con el CMC 32/00. El MERCOSUR también negocia en paralelo con China a nivel de bloque.
- **MERCOSUR-Singapur**: negociaciones en curso desde 2021.
- **MERCOSUR-Canadá**: negociaciones en exploración.
- **MERCOSUR-Indonesia**: diálogos exploratorios.

## Palabras clave

ALADI Uruguay Tratado Montevideo 1980, ACE-18 MERCOSUR ALADI, ACE-35 MERCOSUR Chile Uruguay, ACE-36 MERCOSUR Bolivia Uruguay, ACE-58 MERCOSUR Perú Uruguay, ACE-59 MERCOSUR Colombia Ecuador Uruguay, SGP Sistema Generalizado Preferencias Uruguay, OMC Uruguay GATT, OSD diferencias comerciales Uruguay, Philip Morris CIADI Uruguay tabaco, TLC Uruguay China negociación
