# Indicadores económicos de Uruguay

## Introducción

Los **indicadores económicos** son medidas estadísticas que reflejan el estado y la evolución de la economía. En Uruguay, los principales indicadores son producidos por el **Instituto Nacional de Estadística (INE)**, el **Banco Central del Uruguay (BCU)** y el **Ministerio de Economía y Finanzas (MEF)**. Este archivo describe los indicadores más relevantes, su definición, fuente y evolución histórica general.

---

## Producto Interno Bruto (PIB)

### Definición

El **PIB** mide el valor de mercado de todos los bienes y servicios finales producidos en el territorio de Uruguay durante un período determinado (habitualmente un año). Es el indicador más amplio de la actividad económica.

### Fuente

El INE publica las **Cuentas Nacionales** de Uruguay en las que estima el PIB por el método del gasto, de la producción y del ingreso, siguiendo el **Sistema de Cuentas Nacionales 2008 (SCN 2008)** de las Naciones Unidas.

### Evolución

| Período | Característica principal |
|---|---|
| 1955–1974 | Estancamiento; bajo crecimiento real |
| 1975–1981 | Crecimiento asociado a apertura y reformas |
| 1982 | Caída marcada por crisis de la tablita |
| 1985–1998 | Crecimiento moderado sostenido |
| 1999–2002 | Recesión y caída de ~11% en 2002 |
| 2003–2014 | Crecimiento promedio ~5% anual; reducción de deuda/PIB |
| 2015–2019 | Desaceleración |
| 2020 | Caída de ~6,8% por pandemia COVID-19 |
| 2021–2022 | Recuperación; crecimiento superior al 4% |

---

## Inflación y Precios

### IPC — Índice de Precios al Consumo

El **IPC** mide la variación del costo de una canasta representativa de bienes y servicios consumidos por los hogares uruguayos. Es publicado mensualmente por el **INE**.

El IPC es el indicador de inflación de referencia. Se utiliza para actualizar salarios, contratos de alquiler, la BPC (Base de Prestaciones y Contribuciones) y la Unidad Indexada (UI).

### Evolución histórica de la inflación

| Período | Situación |
|---|---|
| 1970s–1980s | Alta inflación crónica; llegó a superar el 100% anual |
| 1990–1997 | Desinflación gradual mediante anclas cambiarias |
| 1998–2002 | Crisis; inflación alta (llegó al 26% en 2002) |
| 2003–2007 | Estabilización; inflación en descenso |
| 2007–2015 | Inflación en torno al 6–10% anual |
| 2016–2022 | Inflación persistentemente por encima del rango meta (7–10%) |
| 2023 | Proceso de desinflación hacia la banda objetivo del BCU |

### Banda meta del BCU

La banda objetivo de inflación del BCU ha sido históricamente de **3% a 7% anual**. En períodos recientes la inflación ha superado la banda, llevando al BCU a endurecer la política monetaria.

---

## Tipo de cambio

El **tipo de cambio** expresa el precio del dólar estadounidense en pesos uruguayos. El BCU publica diariamente el **tipo de cambio interbancario** (promedio de operaciones entre bancos).

### Evolución del tipo de cambio

| Evento | Efecto |
|---|---|
| Colapso de la tablita (1982) | Devaluación abrupta del peso |
| Crisis de 2002 | El dólar pasó de ~13 a más de 30 pesos en meses |
| Período 2003–2015 | Tipo de cambio relativamente estable (20–30 pesos) |
| Período 2016–2023 | Apreciación del peso relativa; dólar en torno a 35–40 pesos |

---

## Empleo y desempleo

### Fuente

El INE publica la **Encuesta Continua de Hogares (ECH)**, de la cual se derivan las tasas de actividad, empleo y desempleo. Los datos se publican mensualmente.

### Indicadores

| Indicador | Definición |
|---|---|
| Tasa de actividad | Proporción de la población en edad de trabajar que está ocupada o busca trabajo |
| Tasa de empleo | Proporción de la población en edad de trabajar que está ocupada |
| Tasa de desempleo | Proporción de la PEA que busca trabajo y no lo tiene |
| Informalidad | Proporción de ocupados sin cobertura de seguridad social |

### Evolución

La tasa de desempleo llegó al pico de aproximadamente **17%** en 2002. Con la recuperación económica bajó sostenidamente hasta aproximadamente **7–8%** en los años de mayor crecimiento. La pandemia de 2020 provocó un repunte transitorio.

---

## Pobreza e indigencia

El INE mide la **pobreza** y la **indigencia** a través de la metodología de línea de pobreza, aplicada a la ECH.

| Período | Tasa de pobreza aproximada |
|---|---|
| 2002 | ~35% (máximo de la crisis) |
| 2007 | ~25% |
| 2010 | ~18% |
| 2014 | ~9–10% |
| 2019 | ~8% (mínimo histórico) |
| 2020 | Leve aumento por pandemia |
| 2022 | ~9% |

La reducción de la pobreza fue uno de los logros más destacados del período 2005–2019, resultado de la combinación de crecimiento económico, política social (MIDES, Plan de Emergencia, Plan de Equidad) y política de ingresos (Consejo de Salarios, aumento del SMN).

---

## Distribución del ingreso

El **coeficiente de Gini** es el indicador más utilizado para medir la distribución del ingreso. Uruguay redujo su índice de Gini de aproximadamente 0,46 en 2006 a alrededor de 0,39–0,40 en 2019, consolidándose como uno de los países con menor desigualdad de América Latina.

---

## Balanza comercial

La **balanza comercial** registra la diferencia entre las exportaciones y las importaciones de bienes. La **balanza de pagos** incluye además los servicios y las transferencias.

Uruguay ha alternado períodos de superávit y déficit en su balanza comercial de bienes. La entrada en operaciones de las plantas de celulosa (2007, 2022) impulsó significativamente el valor de las exportaciones.

---

## Reservas internacionales

Las **reservas internacionales brutas** son activos externos administrados por el BCU. Se publican semanalmente. Las reservas se expresan en millones de dólares y representan un colchón de liquidez ante shocks externos.

---

## Deuda pública como porcentaje del PIB

La deuda pública/PIB es indicador de sostenibilidad fiscal. Tras la crisis de 2002 (cuando superó el 100% del PIB), Uruguay la redujo progresivamente hasta alrededor del 60–70% del PIB en los años previos a la pandemia.

---

## Palabras clave

indicadores económicos Uruguay, PIB Uruguay, INE Uruguay estadísticas, IPC inflación Uruguay, tipo de cambio Uruguay, desempleo Uruguay ECH, pobreza Uruguay línea pobreza, coeficiente Gini Uruguay, balanza comercial Uruguay, reservas internacionales BCU, deuda pública PIB Uruguay, BPC base prestaciones contribuciones, UI unidad indexada, Encuesta Continua Hogares INE, Cuentas Nacionales INE
