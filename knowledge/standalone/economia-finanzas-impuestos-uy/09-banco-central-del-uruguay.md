# Banco Central del Uruguay (BCU)

## Descripción general

El **Banco Central del Uruguay (BCU)** es el banco central del país. Es una persona jurídica de derecho público, con independencia técnica y operativa para el cumplimiento de sus funciones. Sus objetivos fundamentales son la **estabilidad de precios** (control de la inflación) y la **estabilidad del sistema financiero**.

**Sitio oficial**: [https://www.bcu.gub.uy](https://www.bcu.gub.uy)

## Historia institucional

### Antecedentes

El primer banco público uruguayo fue el **BROU (Banco de la República Oriental del Uruguay)**, fundado en 1896, que durante décadas cumplió funciones de banco central junto con sus actividades comerciales.

### Creación del BCU

El **Banco Central del Uruguay** fue creado por la **Ley 13.608** del 8 de septiembre de **1967**, separando las funciones de banco central de las funciones comerciales del BROU. La creación del BCU respondió a la necesidad de un organismo especializado en la gestión monetaria y la supervisión del sistema financiero.

### Carta Orgánica vigente

La **Ley 16.696** del 30 de marzo de **1994** estableció la Carta Orgánica del BCU vigente, que define sus objetivos, potestades, organización y relaciones con el Poder Ejecutivo. Esta ley le otorgó mayor autonomía operativa para la gestión de la política monetaria.

## Objetivos y funciones

### Objetivos fundamentales

Los objetivos del BCU, según su Carta Orgánica, son:

1. **Estabilidad de precios**: controlar la inflación para preservar el poder adquisitivo de la moneda.
2. **Estabilidad, solvencia y funcionamiento del sistema de pagos y del sistema financiero**.
3. **Regulación de la moneda**: emisión y administración del peso uruguayo.

### Funciones principales

| Función | Descripción |
|---|---|
| Política monetaria | Fijar la Tasa de Política Monetaria (TPM) a través del COPOM |
| Emisión monetaria | Emitir y retirar de circulación billetes y monedas |
| Administración de reservas | Gestionar las reservas internacionales del país |
| Supervisión financiera | Supervisar bancos, AFAPs, aseguradoras y el mercado de valores (SSF) |
| Regulación cambiaria | Supervisar el mercado de cambios |
| Central de riesgos | Administrar la central de riesgos crediticios del sistema financiero |
| Estadísticas | Producir y publicar estadísticas monetarias, financieras y de balanza de pagos |
| Registro de emisores de valores | Regular las emisiones de valores públicos y privados |

## Organización

```
BCU
├── Directorio (5 miembros designados por el Poder Ejecutivo)
│   ├── Presidente del BCU
│   └── 4 Directores
│
├── Comité de Política Monetaria (COPOM)
│
└── Superintendencia de Servicios Financieros (SSF)
    ├── Superintendencia de Instituciones de Intermediación Financiera (SIIF)
    ├── Superintendencia de Seguros y Reaseguros (SSR)
    ├── Superintendencia de AFAPs
    └── Superintendencia de Mercado de Valores (SMV)
```

El Directorio del BCU es designado por el Poder Ejecutivo y sus miembros tienen mandatos escalonados para garantizar la continuidad institucional.

## Comité de Política Monetaria (COPOM)

El **COPOM** es el órgano interno del BCU responsable de las decisiones de política monetaria. Se reúne periódicamente (con frecuencia trimestral o bimensual) para evaluar las condiciones económicas y decidir el nivel de la **Tasa de Política Monetaria (TPM)**.

Las decisiones del COPOM son comunicadas al público mediante comunicados y minutas, contribuyendo a la transparencia y previsibilidad de la política monetaria.

## Política monetaria

### Régimen de metas de inflación

El BCU aplica el **régimen de metas de inflación** desde 2007. Bajo este esquema:

- Se fija una **banda objetivo de inflación** anual (históricamente entre 3% y 7%).
- La **TPM** es el instrumento principal: cuando la inflación proyectada supera el techo de la banda, el BCU sube la TPM para enfriar la economía; cuando está por debajo del piso, la reduce.
- La comunicación de las decisiones y proyecciones de inflación (Informe de Política Monetaria — IPM, publicado trimestralmente) es parte central del régimen.

## Reservas internacionales

El BCU administra las **reservas internacionales** de Uruguay, que son activos externos en monedas convertibles (principalmente dólares), oro y otros instrumentos. Las reservas tienen múltiples funciones:

- Respaldo de la base monetaria
- Capacidad de intervención en el mercado de cambios
- Señal de confianza para los mercados internacionales

Uruguay mantiene reservas internacionales brutas equivalentes a varios meses de importaciones, lo que contribuye a la resiliencia macroeconómica.

## Superintendencia de Servicios Financieros (SSF)

La **SSF** es el órgano del BCU especializado en la supervisión del sistema financiero. Supervisa:

- **Bancos y cooperativas financieras**: solvencia, liquidez, gestión de riesgos (Basilea III adaptado).
- **Empresas de seguros**: requisitos de capital, solvencia II (adaptado).
- **AFAPs**: inversión de los fondos previsionales.
- **Mercado de valores**: emisores, intermediarios, bolsas.
- **Casas de cambio**: control cambiario y antilavado.

## El peso uruguayo y la dolarización

El **peso uruguayo (UYU)** es la moneda oficial de Uruguay. Sin embargo, la economía uruguaya históricamente ha estado altamente dolarizada:

- Gran parte de los contratos, alquileres y precios de bienes durables se expresan en dólares.
- Una proporción importante de los depósitos y créditos del sistema bancario está en dólares.
- La **desdolarización** es un objetivo de política del BCU, promovido mediante instrumentos en pesos e Unidades Indexadas (UI).

## Papel del BCU en la crisis de 2002

Durante la crisis de 2002, el BCU enfrentó el mayor desafío de su historia. La fuga de depósitos en dólares del sistema bancario (más de USD 2.000 millones en meses) obligó a usar las reservas y a recurrir al financiamiento de emergencia del FMI. El BCU liquidó los bancos insolventes, restructuró el BROU e implementó el canje de depósitos.

La crisis llevó a reformas profundas del marco regulatorio: nuevas normas de capital, liquidez, supervisión consolidada y estándares de gobierno corporativo bancario.

## Palabras clave

BCU Banco Central Uruguay, Ley 16.696 Carta Orgánica BCU, COPOM política monetaria Uruguay, TPM tasa política monetaria, metas inflación Uruguay, Superintendencia Servicios Financieros SSF, reservas internacionales Uruguay, peso uruguayo dolarización, BCU supervisión bancaria, Basilea III Uruguay, crisis 2002 BCU, Informe Política Monetaria IPM, Ley 13.608 creación BCU 1967, BROU historia banco central
