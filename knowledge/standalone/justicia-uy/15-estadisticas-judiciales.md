# Estadísticas Judiciales de Uruguay

## Nota metodológica

Las estadísticas judiciales del Uruguay son elaboradas por el **Departamento de Estadística del Poder Judicial**, dependiente de la Suprema Corte de Justicia. El PJ publica anualmente el **Anuario Estadístico Judicial**, disponible en su sitio web (www.poderjudicial.gub.uy, sección Transparencia / Estadísticas).

Los datos de este archivo son de carácter aproximado y estructural. Para datos actualizados y precisos, debe consultarse directamente el Anuario Estadístico Judicial del año correspondiente.

## Estructura del sistema judicial (datos aproximados)

| Instancia | Cantidad aproximada |
|-----------|---------------------|
| Suprema Corte de Justicia | 1 (5 ministros) |
| Tribunales de Apelaciones | 20–25 tribunales (3 ministros c/u) |
| Juzgados Letrados (Montevideo) | ~80 |
| Juzgados Letrados (interior) | ~80 |
| Juzgados de Paz | ~100 |
| **Total juzgados y tribunales** | **~280–290** |

## Recursos humanos del Poder Judicial

| Categoría | Cantidad aproximada |
|-----------|---------------------|
| Total de funcionarios del PJ | Más de 5.000 |
| Magistrados (jueces y ministros) | ~350–400 |
| Actuarios y secretarios letrados | ~200–300 |
| Personal técnico (ITF, mediadores, etc.) | ~200 |
| Personal administrativo y de apoyo | ~4.000 |

## Fiscalía General de la Nación

| Dato | Cantidad |
|------|----------|
| Fiscalías Letradas Departamentales | 33 |
| Departamentos con presencia fiscal | 19 (todos) |
| Fiscalías especializadas en Montevideo | Varias (penal, familia, crimen organizado, DDHH, etc.) |

## Distribución geográfica de la actividad judicial

La distribución de la carga de trabajo judicial en Uruguay es marcadamente desigual:

| Área geográfica | Proporción estimada de la actividad |
|-----------------|-------------------------------------|
| Montevideo | ~60–65% |
| Canelones | ~10–12% |
| Maldonado | ~5–7% |
| Resto del interior (16 departamentos) | ~20–25% |

Esta concentración refleja la distribución poblacional del país: Montevideo y el área metropolitana concentran alrededor del 55–60% de la población total.

## Demanda de servicios judiciales

El Anuario Estadístico del PJ registra anualmente los siguientes indicadores:

- **Asuntos ingresados:** número total de nuevas causas presentadas en todos los juzgados
- **Asuntos egresados:** causas resueltas (por sentencia, archivo, acuerdo u otro mecanismo)
- **Asuntos pendientes:** causas que quedan en tramitación al cierre del período
- **Sentencias dictadas** por tipo de juzgado y materia
- **Audiencias realizadas** (indicador de productividad del sistema oral)
- **Notificaciones electrónicas** enviadas

## Mediación

Los **Centros de Mediación del Poder Judicial** registran anualmente el número de mediaciones iniciadas, los acuerdos alcanzados y las materias involucradas. Las tasas de acuerdo en mediación suelen superar el 50% de los casos en que ambas partes participan efectivamente del proceso.

## Defensoría Pública

La Dirección Nacional de Defensa Pública reporta anualmente la cantidad de personas asistidas, distribuidas por materia (penal, civil, familia, laboral) y por departamento. La reforma procesal penal de 2017–2019 generó un aumento significativo en la demanda de defensores públicos.

## Instituto Técnico Forense (ITF)

El ITF registra anualmente el número de pericias realizadas, distribuidas por especialidad (medicina legal, química, documentología, psicología, etc.) y el tiempo promedio de respuesta.

## Anuario Estadístico Judicial

El **Anuario Estadístico Judicial** es la publicación oficial del Poder Judicial que consolida toda la información estadística del sistema. Se publica con periodicidad anual y cubre:

- Actividad de todos los juzgados y tribunales
- Recursos humanos y presupuestarios
- Estadísticas de mediación
- Estadísticas de la Defensa Pública
- Estadísticas del ITF

**Acceso:** www.poderjudicial.gub.uy (sección Transparencia / Estadísticas / Anuario Estadístico)

## Palabras clave

estadísticas judiciales Uruguay, Anuario Estadístico Judicial Uruguay, Poder Judicial estadísticas Uruguay, asuntos ingresados Uruguay juzgados, magistrados Uruguay cantidad, funcionarios Poder Judicial Uruguay, mediación estadísticas Uruguay, Defensa Pública estadísticas Uruguay, Instituto Técnico Forense pericias Uruguay, juzgados Uruguay distribución territorial, Montevideo concentración actividad judicial, departamento estadísticas tribunales Uruguay, poderjudicial.gub.uy transparencia estadísticas
